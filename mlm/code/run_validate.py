...
parser.add_argument("--model", type = str, help = "model", dest = "model", default = "softmax", required = True)
...
with open(os.path.join(root_path, "models", "seq_len-512", args.model, "config.json"), "r") as f:
    config = json.load(f)

model_config = config["model"]
pretraining_config = config["pretraining_setting"]
gpu_config = config["gpu_setting"]
checkpoint_dir = os.path.join(root_path, "models", "seq_len-512", args.model, "model")
log_dir = os.path.join(root_path, "models", "seq_len-512", args.model, "logs")
...
if args.checkpoint is None:
    log_f_path = os.path.join(log_dir, "valid.log")
    ...
    log_f_path = os.path.join(log_dir, f"validation_output-checkpoint-{args.checkpoint}.log")
...
for epoch in range(start_epoch, end_epoch):
    ...
    print("Model restored from", checkpoint_path, flush = True)
