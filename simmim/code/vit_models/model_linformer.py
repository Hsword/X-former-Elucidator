...
from efficient_attentions.attention_linformer import LinformerSelfAttention
...
class LinformerBlock(nn.Module):
    def __init__(self, config):
        ...
        self.mha = LinformerAttention(config)
...
class LinformerModel(nn.Module):
    def __init__(self, config, use_mask_token=False):
        ...
        for idx in range(self.num_layers):
            setattr(self, f"linformer_vit_block_{idx}", LinformerBlock(config))
    ...
    def forward(self, pixel_values, bool_masked_pos=None):
        ...
        for idx in range(self.num_layers):
            encoder = getattr(self, f"linformer_vit_block_{idx}")
            X = encoder(X, mask)
