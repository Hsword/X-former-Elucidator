...
from efficient_attentions.attention_cosformer import CosformerSelfAttention
...
class CosformerAttention(nn.Module):
    def __init__(self, config):
        ...
        self.attn = CosformerSelfAttention(config)
...
class CosformerBlock(nn.Module):
    def __init__(self, config):
        ...
        self.mha = CosformerAttention(config)
...
class CosformerModel(nn.Module):
    def __init__(self, config, use_mask_token=False):
        ...
        for idx in range(self.num_layers):
            setattr(self, f"cosformer_vit_block_{idx}", CosformerBlock(config))
    ...
    def forward(self, pixel_values, bool_masked_pos=None):
        ...
        for idx in range(self.num_layers):
            encoder = getattr(self, f"cosformer_vit_block_{idx}")
            X = encoder(X, mask)
