...
from efficient_attentions.attention_performer import PerformerSelfAttention
...
class PerformerAttention(nn.Module):
    def __init__(self, config):
        ...
        self.attn = PerformerSelfAttention(config)
...
class PerformerBlock(nn.Module):
    def __init__(self, config):
        ...
        self.mha = PerformerAttention(config)
...
class PerformerModel(nn.Module):
    def __init__(self, config, use_mask_token=False):
        ...
        for idx in range(self.num_layers):
            setattr(self, f"performer_vit_block_{idx}", PerformerBlock(config))
    ...
    def forward(self, pixel_values, bool_masked_pos=None):
        ...
        for idx in range(self.num_layers):
            encoder = getattr(self, f"performer_vit_block_{idx}")
            X = encoder(X, mask)
