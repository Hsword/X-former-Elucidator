...
def profile_train(attn_type):
    ...
    attention_mask = torch.ones((num_iter * batch_size, seq_len)).long()
    ...
    peak_memory_data = inst_per_gpu * (seq_len * (8 + 8) + 8)
    ...
    with open(os.path.join(root_path, "efficiency","nlp_results", f"{attn_type}_glue.json"), 'w') as f:
