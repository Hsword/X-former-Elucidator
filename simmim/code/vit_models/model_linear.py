...
from efficient_attentions.attention_linear import LinearSelfAttention
...
class LinearBlock(nn.Module):
    def __init__(self, config):
        ...
        self.mha = LinearAttention(config)
...
class LinearModel(nn.Module):
    def __init__(self, config, use_mask_token=False):
        ...
        for idx in range(self.num_layers):
            setattr(self, f"linear_vit_block_{idx}", LinearBlock(config))
    ...
    def forward(self, pixel_values, bool_masked_pos=None):
        ...
        for idx in range(self.num_layers):
            encoder = getattr(self, f"linear_vit_block_{idx}")
            X = encoder(X, mask)
