...
class TransNormerModel(nn.Module):
    def __init__(self, config):
        ...
        for idx in range(self.num_layers // 2):
            config["layer"] = idx
            setattr(self, f"transnormer_block_{idx}", TransNormerBlock(config, "diag"))
        for idx in range(self.num_layers // 2, self.num_layers):
            config["layer"] = idx
            setattr(self, f"transnormer_block_{idx}", TransNormerBlock(config, "norm"))
    ...
    def forward(self, input_ids, mask = None):
        ...
        for idx in range(self.num_layers // 2):
            encoder = getattr(self, f"transnormer_block_{idx}")
            X = encoder(X, mask)
        for idx in range(self.num_layers // 2, self.num_layers):
            encoder = getattr(self, f"transnormer_block_{idx}")
            X = encoder(X, mask)
