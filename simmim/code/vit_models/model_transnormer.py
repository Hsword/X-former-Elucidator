...
from efficient_attentions.attention_norm import NormSelfAttention
from efficient_attentions.attention_local import LocalSelfAttention
...
class NormAttention(nn.Module):
    def __init__(self, config):
        ...
        self.attn = NormSelfAttention(config)
...
class TransNormerBlock(nn.Module):
    def __init__(self, config, attn_type):
        ...
        self.attn = attn_type
        if attn_type == "norm":
            self.mha = NormAttention(config)
        elif attn_type == "diag":
            self.mha = DiagAttention(config)
        else:
            assert False
...
class TransNormerModel(nn.Module):
    def __init__(self, config, use_mask_token=False):
        ...
        self.embeddings = Embeddings(config, use_mask_token=use_mask_token, drop_last=self.drop_last)
        self.norm = nn.LayerNorm(self.hidden_dim)

        for idx in range(self.num_layers // 2):
            setattr(self, f"transnormer_vit_block_{idx}", TransNormerBlock(config, "diag"))
        for idx in range(self.num_layers // 2, self.num_layers):
            setattr(self, f"transnormer_vit_block_{idx}", TransNormerBlock(config, "norm"))
    ...
    def forward(self, pixel_values, bool_masked_pos=None):
        ...
        for idx in range(self.num_layers // 2):
            encoder = getattr(self, f"transnormer_vit_block_{idx}")
            X = encoder(X, mask)
        for idx in range(self.num_layers // 2, self.num_layers):
            encoder = getattr(self, f"transnormer_vit_block_{idx}")
            X = encoder(X, mask)
