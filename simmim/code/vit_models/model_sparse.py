...
from efficient_attentions.attention_sparse import SparseSelfAttention
...
class SparseAttention(nn.Module):
    def __init__(self, config):
        ...
        self.attn = SparseSelfAttention(config)
...
class SparseBlock(nn.Module):
    def __init__(self, config):
        ...
        self.mha = SparseAttention(config)
...
class SparseModel(nn.Module):
    def __init__(self, config, use_mask_token=False):
        ...
        for idx in range(self.num_layers):
            setattr(self, f"sparse_vit_block_{idx}", SparseBlock(config))
    ...
    def forward(self, pixel_values, bool_masked_pos=None):
        ...
        for idx in range(self.num_layers):
            encoder = getattr(self, f"sparse_vit_block_{idx}")
            X = encoder(X, mask)
